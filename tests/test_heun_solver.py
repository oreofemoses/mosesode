import numpy as np
import pytest
from moses_ode.solvers.heun import heun_solver


def test_heun_solver_scalar():
    """Test Heun's solver with a simple scalar ODE dy/dt = -2y."""

    def f(t, y):
        return -2 * y

    t0, y0, h, t_end = 0, 1.0, 0.01, 1.0
    t_values, y_values = heun_solver(f, t0, y0, h, t_end)
...
def test_heun_solver_vector():
    """Test Heun's solver with a system of ODEs."""
...
def test_heun_solver_large_step():
    """Test Heun's solver with a large step size."""
...
def test_heun_solver_edge_case():
    """Test Heun's solver with t0 == t_end (should return initial condition)."""
...
def test_heun_solver_small_step():
    """Test Heun's solver with a very small step size."""

    def f(t, y):
        return -2 * y

    t0, y0, h, t_end = 0, 1.0, 0.001, 1.0
    t_values, y_values = heun_solver(f, t0, y0, h, t_end)
...
def test_heun_solver_nonlinear():
    """Test Heun's solver with a non-linear ODE dy/dt = -y^2."""
